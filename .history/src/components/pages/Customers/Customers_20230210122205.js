...
const Customers = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const AddEmployee = async (e) => {
      ...
      const formD = { employeeName , Mobile , Email , Address , EmployeeCode , EmployeeRole , OfficeLocation  , Department , Designation , BasicSalary , State , District , Taluka  }

      try{
        const { data } = await axios.post("https://nxyf2bcbj9.execute-api.ap-south-1.amazonaws.com/dev/api/v1/emp/add")
      }catch(err){
        console.log(err)
      }
    } 
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Select aria-label="Default select example" required>
                    ...
                    {EofficeLocation?.data?.map((i, index) => (
                      <>
                        <option>{i.officeLocation} </option>
                      </>
                    ))}
                  </Form.Select>
                </Form.Group>

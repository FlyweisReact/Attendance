  ...
  const fetchHandler = async () => {
    try {
      const { data } = await axios.get(
        "http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/notificationRouter/getallusers"
      );
      setData(data);
    } catch (err) {
      console.log(err);
  ...
  const AddNote = async (e) => {
    ...
    try {
      const data = await axios.post(
        "http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/notificationRouter/sendNotificationstouser",
        {
          role,
          message,
        }

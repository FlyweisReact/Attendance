...
const PushNotification = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const AddLocation = async (e) => {
      e.preventDefault()
      try{
        const {data } = await axios.post('https://nxyf2bcbj9.execute-api.ap-south-1.amazonaws.com/dev/api/v1/admin/officeLocations' , {officeLocation})
  ...
  const deleteLocation = async(id) =>{
    try{
      const { data } = await axios.delete(`https://nxyf2bcbj9.execute-api.ap-south-1.amazonaws.com/dev/api/v1/admin/officeLocations/${id}` , {
        headers : {
          Authorization : `Bearer ${token}`
        }
      })
          ...
          <tbody>
            {data?.data?.map((i, index) => (
              <tr key={index}>
                ...
                <td>
                  <AiFillDelete color="red" cursor={"pointer"} onClick={() => deleteLocation(i._id)}  />
                </td>
              </tr>
            ))}
          </tbody>
